...
class RequestReader
{
     ...
     */
    public function getBody(Request $request, ?string $readerType = null): mixed
    {
        $data = (string) $request->getContent();

        $payload = Payload::create($data, $request->headers->get('Content-Type'));
        if ($readerType !== null) {
    ...
    public function getBodyAs(Request $request, mixed $schema, ?ValidatorInterface $validator = null, ?string $readerType = null): mixed
    {
        $data = (string) $request->getContent();
